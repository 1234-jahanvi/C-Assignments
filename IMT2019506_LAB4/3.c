/*INPUT: Comma seperated ten integers in sorted order into an array and an integer to be searched .
...
int main()
{
	int n[10];					/*Declaration of the array of ten integers*/
	int x;						/*x=the number to be searched*/
	scanf("%d",&n[0]);				/*Taking the input of the first integer element of the array*/
	for(int i=1;i<10;i++)
	{
		scanf(", %d",&n[i]);			/*Taking the input of elements of the array except the first element*/
	}
	...
	int lower=0;					/*lower=the lowest index number of the array*/
	int upper=9;					/*upper=the largest index number of the array*/
	while(lower<=upper)
	{
		count++;
		int mid=(lower+upper)/2;		/*mid=the middle index number of the array*/
		if(x < n[mid])
		{
			upper=mid-1;
		}
